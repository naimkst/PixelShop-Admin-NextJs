...
import { createProduct, getById, productUpdate } from "../../services/product";
import IsLoading from "../../components/Loading/isloading";

function EditProduct() {
  const router = useRouter();
  const [loading, setLoding] = useState(false);
  const [setImage, setImageUrl]: any = useState(false);
  ...
  const onSubmit: SubmitHandler<any> = async (data) => {
    ...
    try {
      ...
      const response = await productUpdate(router.query.id, formData);
  ...
  const getPrd = async () => {
    setLoding(true);
    const response = await getById(router.query.id);
    console.log(response);
    setData(response);
    reset(response);
    setLoding(false);
                  ...
                  {loading ? (
                    ...
                    <>
                      <div className="mt-10 px-7">
                        ...
                        <div className="grid w-full grid-cols-1 lg:grid-cols-2 md:grid-cols-1 gap-7 mt-7">
                          <div>
                            ...
                            <input
                              className="w-full p-3 mt-4 h-14 border border-gray-300 rounded outline-none focus:bg-gray-50"
                              {...register("title", {
                            ...
                            />
                            {errors.title && (
                              <p className="mt-3 text-xs leading-3 text-red-500">
                      ...
                      <div>
                          ...
                          <div className="rounded-lg shadow-xl bg-gray-50 w-[70%]">
                            <div className="m-4">
                              ...
                              <div className="flex items-center justify-center w-full">
                                <label className="flex flex-col w-full h-48 border-4 border-dashed hover:bg-gray-100 hover:border-gray-300  items-center justify-center">
                                  <div className="flex flex-col items-center justify-center">
                                    {setImage ? (
                                      <img
                                        src={setImage}
                                        alt=""
                                        className="w-72 h-44 object-cover flex items-center justify-center rounded-md"
                                      />
                                    ) : (
                                      <>
                                        <img
                                          src={`${process.env.NEXT_PUBLIC_PATH}${getData?.image}`}
                                          alt=""
                                          className="w-72 h-44 object-cover flex items-center justify-center rounded-md"
                                        />
                                      </>
                        ...
                        {errors.image && (
                          <p className="mt-3 text-xs leading-[15px] flex justify-center text-red-500">
                            {errors.image?.message}
                          </p>
                        )}
                      </div>

                      <div className=" w-[50%] items-center justify-center m-auto mt-10">
                        {/* <p className="text-base font-medium leading-none text-gray-800">
                          Feature Product
                        </p> */}
                        {/*-Dropdown*/}
                        ...
                        <select
                          defaultValue=""
                          className="
                              relative flex mt-4  w-full px-3 py-4 h-14 dropbtn-one
                              border border-gray-300 rounded outline-none dropdown-one text-sm font-medium text-gray-600
                              "
                          aria-label="Default select example"
                          {...register("featured", {
